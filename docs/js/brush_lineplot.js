...
let svg_line = d3.select("#lineplot").append("svg").attr("width", 500).attr("height", 300)

let y_scale = d3.scaleLinear().domain([10,7]).range([50,250]);
let y_axis = d3.axisRight().scale(y_scale);
svg_line.append("g").attr("transform", "translate(50, 0)").call(y_axis);

let x_scale = d3.scaleTime().domain([new Date(2015, 8, 1), new Date(2017, 8, 31)]).range([50, 450]);
...
svg_line.append("rectangle").attr("width", 500).attr("height", 300)
.attr("style", "fill:rgb(255,255,255);stroke-width:3;stroke:rgb(0,0,0)");

const brush = d3.brushX()
.extent([[50, 50], [450, 250]]);
brush.on("end", brushended);

svg_line.append("g").call(brush).call(brush.move, [50, 450]);
